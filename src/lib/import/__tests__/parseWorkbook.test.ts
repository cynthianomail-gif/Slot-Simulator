...
describe('warnings / info — 透明回報', () => {
  it('回報全零賠付（會轉但不中獎）', () => {
    expect(warnings.some((w) => w.includes('賠付皆為 0'))).toBe(true);
  });
  ...
  it('回報不支援的分頁（Combo/Puzzle/Discard/GLOBAL_MAX）', () => {
    const all = warnings.join('\n');
    expect(all).toContain('08_Combo_Weights');
    expect(warnings.some((w) => w.includes('GLOBAL_MAX'))).toBe(true);
  });

  it('回報 Max_Count 已略過', () => {
    expect(warnings.some((w) => w.includes('Max_Count'))).toBe(true);
  });
